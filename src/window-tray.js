/* eslint-disable */
const { BrowserWindow, ipcMain, Tray, screen } = require("electron");
let tray = undefined;
...
function calculateWindowPosition() {
  ...
  let trayPos = 4, x, y; // 1:top-left 2:top-right 3:bottom-left 4.bottom-right
  ...
  case 4: // for BOTTOM - RIGHT
    ...
    if (DEFAULT_MARGIN.y < 40) {
      y = Math.floor(
        trayBounds.y - height - DEFAULT_MARGIN.y + trayBounds.height / 2
      );
    } else {
      y = Math.floor(
        trayBounds.y - height - DEFAULT_MARGIN.y + DEFAULT_MARGIN.y / 2 + trayBounds.height / 2
      );
    }
    break;
  }
